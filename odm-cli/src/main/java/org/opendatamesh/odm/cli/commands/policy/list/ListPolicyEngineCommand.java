...
public class ListPolicyEngineCommand implements Runnable {
    ...
    public void run() {
        try {
            ResponseEntity<PagedPolicyEngineResource> engineResourceResponseEntity =
            ...
            if(engineResourceResponseEntity.getStatusCode().equals(HttpStatus.OK)) {
                List<PolicyEngineResource> engines = engineResourceResponseEntity.getBody().getContent();
                if (engines.size() == 0)
                    System.out.println("[]");
