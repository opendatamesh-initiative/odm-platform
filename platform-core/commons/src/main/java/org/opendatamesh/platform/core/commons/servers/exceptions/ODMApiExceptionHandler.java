...
// @ControllerAdvice
public class ODMApiExceptionHandler extends ResponseEntityExceptionHandler {


	@ExceptionHandler({ODMApiException.class})
	protected ResponseEntity<Object> handleOpenDataMeshException(ODMApiException e, WebRequest request) {
	...
	protected ResponseEntity<Object> handleExceptionInternal(Exception e, Object body, HttpHeaders headers, HttpStatus status, WebRequest request) {
		if(!ODMApiException.class.isAssignableFrom(e.getClass())) {
		...
		if(HttpMessageNotReadableException.class.isAssignableFrom(e.getClass())) { // if something went erong while parsing the request body...
			...
			body = new ErrorRes(
				status.value(), 
				ODMApiCommonErrors.SC400_00_REQUEST_BODY_IS_NOT_READABLE, 
				message, url);
			...
			body = new ErrorRes(
				status.value(), 
				ODMApiCommonErrors.SC415_01_REQUEST_MEDIA_TYPE_NOT_SUPPORTED, 
				message, url);
			...
			body = new ErrorRes(
				status.value(), 
				ODMApiCommonErrors.SC406_01_REQUEST_ACCEPTED_MEDIA_TYPES_NOT_SUPPORTED, 
				message, url);
