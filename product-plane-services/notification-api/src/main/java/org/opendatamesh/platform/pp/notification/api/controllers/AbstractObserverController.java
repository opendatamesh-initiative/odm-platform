...
public abstract class AbstractObserverController implements ObserverController {
    ...
    @ApiResponses(value = {
            @ApiResponse(
                    ...
                    content = @Content(
                            ...
                            examples = {@ExampleObject(name = "observer", value = EXAMPLE_OBSERVER)}
                    )
            ),
            @ApiResponse(
                    responseCode = "400",
    ...
    @Operation(
            summary = "Get all Observers",
            description = "Get all the registered Observers paginated"
    )
    @ResponseStatus(HttpStatus.OK)
    @ApiResponses(value = {
            @ApiResponse(
    ...
    @ApiResponses(value = {
            @ApiResponse(
                    responseCode = "200",
                    description = "The requested Observer was deleted successfully"
            ),
            @ApiResponse(
                    responseCode = "404",
                    description = "[Not Found](https://www.rfc-editor.org/rfc/rfc9110.html#name-404-not-found)"
